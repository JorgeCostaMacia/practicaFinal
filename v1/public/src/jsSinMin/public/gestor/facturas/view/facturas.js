...
function injectLineas(lineasFactura, factura){
    ...
    tableLineas += '<tr><td></td><td></td><td></td><td></td><td></td><td></td><td><button type="button" id="procesar" name="procesar" class="form-control btn-warning" value="" >Procesar</button></td><td></td></tr>';
...
function showDescarga(cliente, factura){
    ...
    doc.text(150,54,'Codigo albaran : ' + factura["cod_albaran"]);
    doc.text(150,58,'Cod cliente: ' +  factura["cod_cliente"]);
    doc.text(150,62,'Fecha: ' + facturasApp.formatDate(factura["fecha"]));
    doc.text(150,66,'Estado: ' + factura["estado"]);

    let columns = ["Codigo", "Articulo", "Precio", "Cantidad", "Descuento", "Iva", "Total", "Estado"];
    let data = [];

    for(let i = 0; i < lineasFacturas.length; i++){
        let acceso = [lineasFacturas[i]["cod_linea"], lineasFacturas[i]["cod_articulo"] + ' - ' + lineasFacturas[i]["nombre_articulo"], lineasFacturas[i]["precio"].replace(".", ","), lineasFacturas[i]["cantidad"], lineasFacturas[i]["descuento"].replace(".", ","), lineasFacturas[i]["iva"].replace(".", ","), lineasFacturas[i]["total"].replace(".", ","), lineasFacturas[i]["estado"]];

...
function validateCifDni(cif_dni){
    let numero = cif_dni.substr(0,cif_dni.length-1);
    let letra = cif_dni.substr(cif_dni.length-1,1);
    numero = numero % 23;
    let letras='TRWAGMYFPDXBNJZSQVHLCKET';
    let letraAux = letras.substring(numero,numero+1);

    return letraAux === letra.toUpperCase();
}

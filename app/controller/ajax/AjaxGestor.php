...
class AjaxGestor extends DependencyCRUD{
        ...
        if (isset($_POST["borrar"])) {
            ...
            if ($this->dataContent->getSuccess() && count($this->dataContent->getLineasFacturas()) > 0) {
                ...
                if ($this->dataContent->getSuccess() && count($values) > 0) {
                    $values = $this->getLineasAlbaranesCRUD()->updateEstadoFacturas($this->connection, $this->dataContent, "pendiente", $values);
                }
                if ($this->dataContent->getSuccess() && count($values[0]) > 0) {
                    $this->getActividadCRUD()->prepareLineas($this->connection, $this->dataContent, $_POST["cod_albaran"], $values, 'lineas_albaranes', 'pendiente', trim($_POST['cod_gestor']), 'gestor');
                }
                if ($this->dataContent->getSuccess()) {
                ...
                if ($this->dataContent->getSuccess()) {
                    $this->getAlbaranesCRUD()->update($this->connection, $this->dataContent, 'estado="pendiente"');
                }
                if ($this->dataContent->getSuccess()) {
                    $this->getActividadCRUD()->insert($this->connection, $this->dataContent, $_POST["cod_albaran"], "albaranes", "pendiente", trim($_POST['cod_gestor']), 'gestor');
                }
            }
        }
        else {
    ...
    public function procesarFactura(){
       $this->connection->startTransaction();
        ...
        if ($this->dataContent->getSuccess()) {
            $this->getActividadCRUD()->insert($this->connection, $this->dataContent, $_POST["cod_factura"], "facturas", $_POST["estado"], trim($_POST['cod_gestor']), 'gestor');
        }
        if ($this->dataContent->getSuccess()) {

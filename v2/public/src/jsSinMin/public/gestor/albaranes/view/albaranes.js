...
function showDescarga(cliente, albaran){
    ...
    doc.text(150,54,'Cod cliente: ' +  albaran["cod_cliente"]);
    doc.text(150,58,'Fecha: ' + albaranesApp.formatDate(albaran["fecha"]));
    doc.text(150,62,'Estado: ' + albaran["estado"]);

    let columns = ["Codigo", "Pedido", "Albaran", "Articulo", "Precio", "Cantidad", "Descuento", "Iva", "Total", "Estado"];
    let data = [];

    for(let i = 0; i < lineasAlbaranes.length; i++){
        let acceso = [lineasAlbaranes[i]["cod_linea"],lineasAlbaranes[i]["cod_pedido"],  lineasAlbaranes[i]["cod_albaran"], lineasAlbaranes[i]["cod_articulo"] + ' - ' + lineasAlbaranes[i]["nombre_articulo"], lineasAlbaranes[i]["precio"].replace(".", ","), lineasAlbaranes[i]["cantidad"], lineasAlbaranes[i]["descuento"].replace(".", ","), lineasAlbaranes[i]["iva"].replace(".", ","), lineasAlbaranes[i]["total"].replace(".", ","), lineasAlbaranes[i]["estado"]];

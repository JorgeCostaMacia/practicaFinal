...
function injectLineas(lineasFactura, factura){
    ...
    for(let i = 0; i < lineasFactura.length; i++){
        ...
        tableLineas +=
            ...
            '<td>' + lineasFactura[i]["descuento"].replace(".", ",") + '</td>' +
            '<td>' + lineasFactura[i]["iva"].replace(".", ",") + '</td>' +
            '<td>' + lineasFactura[i]["total"].replace(".", ",") + '</td>' +
...
function showDescarga(cliente, factura){
    ...
    doc.text(150,54,'Cod cliente: ' +  factura["cod_cliente"]);
    doc.text(150,58,'Fecha: ' + facturasApp.formatDate(factura["fecha"]));
    doc.text(150,62,'Estado: ' + factura["estado"]);

    let columns = ["Codigo", "Factura", "Albaran", "Articulo", "Precio", "Cantidad", "Descuento", "Iva", "Total", "Estado"];
    let data = [];

    for(let i = 0; i < lineasFacturas.length; i++){
        let acceso = [lineasFacturas[i]["cod_linea"], lineasFacturas[i]["cod_factura"],lineasFacturas[i]["cod_albaran"], lineasFacturas[i]["cod_articulo"] + ' - ' + lineasFacturas[i]["nombre_articulo"], lineasFacturas[i]["precio"].replace(".", ","), lineasFacturas[i]["cantidad"], lineasFacturas[i]["descuento"].replace(".", ","), lineasFacturas[i]["iva"].replace(".", ","), lineasFacturas[i]["total"].replace(".", ","), lineasFacturas[i]["estado"]];
        total += 1 * lineasFacturas[i]["total"];
        data.push(acceso);
    }
    doc.autoTable(columns,data, {styles: {fontSize: 5, overflow: 'linebreak'}, margin:{ top: 89 }});

    doc.setFontSize(8);
    doc.text(20,70, facturasApp.getDate() + ' Elche');
    doc.setFontType("bold");
    doc.text(20,74,('Total: ' +  total + "€").replace(".", ","));
    doc.text(20,78,('Descuento: ' +  factura["descuento"] + "%").replace(".", ","));
    doc.text(20,82,('Total con descuento: ' +  (total - (total * factura["descuento"] / 100)) + "€").replace(".", ","));

...
function restoreFacturas(){
    $('#tableLineas').empty();
    $('#tableFacturas').attr('hidden', false);
}

function cleanLineas(){
    $('#tableLineas').empty();
}
function injectFacturas(facturas){
    for(let i = 0; i < facturas.length; i++){
        $('#tbody').append(
            '<tr><td>' + facturas[i]["cod_factura"] + '</td>' +
            '<td>' + facturas[i]["cod_cliente"] + '-' + facturas[i]["nombre_cliente"] + '</td>' +
            '<td>' + facturasApp.formatDate(facturas[i]["fecha"]) + '</td>' +
            '<td>' + facturas[i]["descuento"].replace(".", ",") + '</td>' +
            '<td>' + facturas[i]["estado"] + '</td>' +
            '<td><input type="button" id="cod_factura-' + facturas[i]["cod_factura"] + '" value="' + facturas[i]["lineas"] + '" class="form-control btn-primary"></td></tr>'
        );
    }
}
...
function injectLineas(lineasFactura, factura){
    let tableLineas = '<form id="formLineas" name="formLineas">' +
        ...
        '<input type="hidden" id="cod_factura" name="cod_factura" value="' + cod_factura + '">' +
        '<input type="hidden" id="cod_albaran" name="cod_albaran" value="' + factura["cod_albaran"] + '">' +
        '<h2>Factura</h2>' +
        ...
        '</thead>' +
        '<tbody>' +
        '<tr><td>' + factura["cod_factura"] + '</td>' +
        '<td>' + factura["cod_cliente"] + '-' + factura["nombre_cliente"] + '</td>' +
        '<td>' + facturasApp.formatDate(factura["fecha"]) + '</td>' +
        '<td id="total"></td>';

        if(factura["estado"] === "pendiente"){
            tableLineas += '<td><div class="material-switch">'+
                '<input id="estado" name="estado" type="checkbox" value="estado"/>'+
                '<label for="estado" class="label-warning"></label>'+
                '</div></td>';

            tableLineas += '<td><div class="material-switch">'+
                '<input id="borrar" name="borrar" type="checkbox" value="borrar"/>'+
                '<label for="borrar" class="label-danger"></label>'+
                '</div></td>';

            tableLineas += '<td><input type="number" name="descuento" id="descuento" value="' + factura["descuento"] + '" class="form-control" min="0" max="100" step="1"></td>';
        }
        else if(factura["estado"] === "procesado"){
            tableLineas += '<td><div class="material-switch">'+
                '<input id="estado" name="estado" type="checkbox" value="estado" checked/>'+
                '<label for="estado" class="label-warning"></label>'+
                '</div></td>';

            tableLineas += '<td><div class="material-switch">'+
            '<input id="borrar" name="borrar" type="checkbox" value="borrar" disabled/>'+
            '<label for="borrar" class="label-danger"></label>'+
            '</div></td>';

            tableLineas += '<td><input type="number" name="descuento" value="' + factura["descuento"] + '" class="descuentos form-control" min="0" max="1" step="0.01" disabled></td>';

        }
    tableLineas += '<tr><td></td><td></td><td></td><td></td><td></td><td></td><td><button type="button" id="procesar" name="procesar" class="form-control btn-warning" value="" >Estado</button></td><td></td></tr>';
    tableLineas += '<tr><td></td><td></td><td></td><td></td><td></td><td></td><td><button type="button" id="actualizar" name="actualizar" class="form-control btn-warning" value="" >Actualizar</button></td><td></td></tr>';
    tableLineas += '<tr><td></td><td></td><td></td><td></td><td></td><td></td><td><button type="button" id="descarga" name="descarga" class="form-control btn-success" value=""><span class="glyphicon glyphicon-download" aria-hidden="true"></span> PDF</button></td></tr>';
    tableLineas += '<tr><td></td><td></td><td></td><td></td><td></td><td></td><td><button type="button" id="buttonVolver" name="buttonVolver" class="form-control btn-primary">Volver</button></td></tr>';


        tableLineas += '</tbody></table></div>' +
        ...
        '</thead>' +
        '<tbody>';
    for(let i = 0; i < lineasFactura.length; i++){
        total += (1 * lineasFactura[i]["total"]);

        tableLineas +=
            '<tr><td>' + lineasFactura[i]["cod_linea"] + '</td>' +
            '<td>' + lineasFactura[i]["cod_articulo"] + '-' + lineasFactura[i]["nombre_articulo"] + '</td>' +
            '<td>' + lineasFactura[i]["precio"] + '</td>' +
            '<td>' + ("" + ((1 * lineasFactura[i]['descuento']).toFixed(2))).replace(".", ",")+ '</td>' +
            '<td>' + lineasFactura[i]["iva"].replace(".", ",") + '</td>' +
            '<td>' + ("" + ((1 * lineasFactura[i]['total']).toFixed(2))).replace(".", ",")+ '</td>' +
            '<td>' + lineasFactura[i]["estado"] + '</td>' +
            '<td>' + lineasFactura[i]["cantidad"] + '</td></tr>';
    }

    tableLineas += '</tbody></table></div></form>';

    $('#tableFacturas').attr('hidden', true);
    $('#tableLineas').append(tableLineas);

    total -= (1 * total * factura["descuento"] / 100);

    $('#total').append(("" + ("" + ((1 * total).toFixed(2)))).replace(".", ","));
}


function showDescarga(cliente, factura){
    ...
    doc.text(20,49,'Cod cliente: ' +  cliente["cod_cliente"]);
    doc.text(20,53,'CIF / DNI : ' + cliente["cif_dni"]);
    doc.text(20,57,'Razon social: ' +  cliente["razon_social"]);
    doc.text(20,61,'Domicilio social: ' + cliente["domicilio_social"]);
    doc.text(20,65,'Ciudad: ' + cliente["ciudad"]);

    doc.setFontSize(15);
    doc.text(150,45,"Factura");
    doc.setFontSize(8);
    doc.text(150,50,'Codigo factura: ' + factura["cod_factura"]);
    doc.text(150,54,'Fecha: ' + facturasApp.formatDate(factura["fecha"]));
    doc.text(150,58,'Estado: ' + factura["estado"]);

    let columns = ["Articulo", "Precio", "Cantidad", "Descuento", "Iva", "Total", "Estado"];
    let data = [];

    for(let i = 0; i < lineasFacturas.length; i++){
        let acceso = [lineasFacturas[i]["cod_articulo"] + ' - ' + lineasFacturas[i]["nombre_articulo"], ("" + ((1 * lineasFacturas[i]["precio"]).toFixed(2))).replace(".", ","), lineasFacturas[i]["cantidad"], lineasFacturas[i]["descuento"].replace(".", ","), lineasFacturas[i]["iva"].replace(".", ","), ("" + ((1 * lineasFacturas[i]["total"]).toFixed(2))).replace(".", ","), lineasFacturas[i]["estado"]];
        total += 1 * lineasFacturas[i]["total"];
        data.push(acceso);
    }
    doc.autoTable(columns,data, {styles: {fontSize: 5, overflow: 'linebreak'}, margin:{ top: 93 }});

    doc.setFontSize(8);
    doc.text(20,74, facturasApp.getDate() + ' Elche');
    doc.setFontType("bold");
    doc.text(20,78,('Total: ' +  ("" + ((1 * total).toFixed(2))) + "€").replace(".", ","));
    doc.text(20,82,('Descuento: ' +  factura["descuento"] + "%").replace(".", ","));
    doc.text(20,86,('Total con descuento: ' +  ("" + ((1 * (total - (total * factura["descuento"] / 100))).toFixed(2))) + "€").replace(".", ","));

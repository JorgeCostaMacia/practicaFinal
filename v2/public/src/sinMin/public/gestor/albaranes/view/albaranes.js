...
function injectTitleAlbaranes(){
    $('#title').empty();
    $('#title').append('Albaranes');
}

function injectTitleCarrito(){
    $('#title').empty();
    $('#title').append('Carrito procesar');
}
...
function injectCarrito(albaranes){
    for(let i = 0; i < albaranes.length; i++){
        $('#tbody').append(
            '<tr><td>' + albaranes[i]["cod_albaran"] + '</td>' +
            '<td>' + albaranes[i]["cod_cliente"] + '-' + albaranes[i]["nombre_cliente"] + '</td>' +
            '<td>' + albaranesApp.formatDate(albaranes[i]["fecha"]) + '</td>' +
            '<td>' + albaranes[i]["estado"] + '</td>' +
            '<td>' + albaranes[i]["lineas"] + '</td></tr>'
        );
    }
    $('#tbody').append(
        '<tr><td></td><td></td><td></td><td></td><td><button type="button" id="procesar" name="procesar" class="form-control btn-warning" value="" >Procesar</button></td></tr>');
...
function injectLineas(lineas){
    let tableLineas = '<form id="formLineas" name="formLineas">' +
        ...
        '<h2>Albaran ' + lineas[0]["cod_albaran"] +' - Pedido ' + lineas[0]["cod_pedido"] + '</h2>' +
    ...
    for(let i = 0; i < lineas.length; i++){
        tableLineas +=
            ...
            '<td>' + ("" + ((1 * lineas[i]['total']).toFixed(2))).replace(".", ",")+ '</td>' +
    ...
    tableLineas += '<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><button type="button" id="agregar" name="agregar" class="form-control btn-warning" value="" >Agregar</button></td><td></td></tr>';
...
function showDescarga(cliente, albaran){
    ...
    doc.text(20,49,'Cod cliente: ' +  albaran["cod_cliente"]);
    ...
    doc.text(150,54,'Fecha: ' + albaranesApp.formatDate(albaran["fecha"]));
    doc.text(150,58,'Estado: ' + albaran["estado"]);

    let columns = ["Articulo", "Precio", "Cantidad", "Descuento", "Iva", "Total", "Estado"];
    let data = [];

    for(let i = 0; i < lineasAlbaranes.length; i++){
        let acceso = [lineasAlbaranes[i]["cod_articulo"] + ' - ' + lineasAlbaranes[i]["nombre_articulo"], ("" + ((1 * lineasAlbaranes[i]["precio"]).toFixed(2))).replace(".", ","), lineasAlbaranes[i]["cantidad"], lineasAlbaranes[i]["descuento"].replace(".", ","), lineasAlbaranes[i]["iva"].replace(".", ","), ("" + ((1 * lineasAlbaranes[i]["total"]).toFixed(2))).replace(".", ","), lineasAlbaranes[i]["estado"]];
        total += 1 * lineasAlbaranes[i]["total"];
        data.push(acceso);
    }
    doc.autoTable(columns,data, {styles: {fontSize: 5, overflow: 'linebreak'}, margin:{ top: 85 }});

    doc.setFontSize(8);
    doc.text(20,74, albaranesApp.getDate() + ' Elche');
